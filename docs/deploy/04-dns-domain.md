...
- `https://elixio.digital` → Cloudflare Pages
- `https://api.elixio.digital` → Railway API
- Email DNS (Resend) is verified
- HSTS is on

## 4.1 Add `elixio.digital` to Cloudflare

1. Go to https://dash.cloudflare.com → **Add a site** → `elixio.digital`.
...
4. Go to your **Hostinger domain settings** (or wherever you bought `elixio.digital`) → change the nameservers to the Cloudflare ones.
...
2. Enter `elixio.digital`. Cloudflare auto-detects the CNAME you just created.
3. Wait for the SSL certificate to provision (~2 minutes for Cloudflare-managed).
4. Repeat for `www.elixio.digital` (redirect to apex).

## 4.4 Add the custom domain to Railway

1. Railway → API service → **Settings** → **Domains** → **Add Domain**.
2. Enter `api.elixio.digital`. Railway will give you the target (e.g., `api.elixio.digital.up.railway.app`) — but because you already have the CNAME pointing at `*.up.railway.app`, you can just confirm.
...
| --- | --- | --- |
| Railway API | `ELIXIO_API_URL` | `https://api.elixio.digital` |
| Railway API | `ELIXIO_WEB_URL` | `https://elixio.digital` |
| Cloudflare Pages (Production) | `NEXT_PUBLIC_API_URL` | `https://api.elixio.digital` |
| EAS `eas.json` (preview + production) | `EXPO_PUBLIC_API_URL` | `https://api.elixio.digital` |
| Google OAuth | Authorized redirect URIs | add `https://api.elixio.digital/v1/auth/oauth/google/callback` |
| GitHub OAuth App | Authorization callback URL | add `https://api.elixio.digital/v1/auth/oauth/github/callback` |
...
- `https://elixio.digital` shows the Elixio Digital landing page.
- `https://api.elixio.digital/v1/health` returns `ok`.
- `dig api.elixio.digital` returns the Railway hostname.
- Resend shows the green check for `elixio.digital`.
