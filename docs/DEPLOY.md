...
3. Wire up DNS so `api.elixiodigital.com` and `elixiodigital.com` resolve.
...
- `https://elixiodigital.com` loads the web app.
- `https://api.elixiodigital.com/v1/health` returns `{ "status": "ok" }`.
